...
define_function write_ir()		//This function is called in DEFINE_START to pass the CAM_PTZ structure to the module
{
	local_var	long lPos
	local_var	slong slReturn
	local_var	slong slFile
	...
	// Convert To Binary
	lPos = 1
	slReturn = variable_to_string(ir, sBINString, lPos)
	// Save Structure to Disk - Binary
	slFile = file_open('BinaryIREncode.xml', 2)
	slReturn = file_write(slFile, sBINString, length_string(sBINString))
	slReturn = file_close(slFile)
}
