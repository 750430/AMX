...
define_function show_level(integer nI)
{
	select
	{
		active(nI>0 and nI<=30):	if(mxr_vol[nI].enabled) send_level dvTP[nI],1,mxr_vol[nI].lvl
		active(nI>30 and nI<=60):	if(mxr_vol[nI].enabled) send_level dvTP[nI-30],2,mxr_vol[nI].lvl
		active(nI>60 and nI<=90):	if(mxr_vol[nI].enabled) send_level dvTP[nI-60],3,mxr_vol[nI].lvl
	}
	if(mxr_vol[nI].enabled) send_level vdvMixer_FB[nI],1,mxr_vol[nI].lvl
}
...
define_function update_tp()
{
	for(x=1;x<=max_length_array(MXR_VOL);x++)
	{
		...
		if(mxr_vol[x].enabled) 
		{
			select
			{
				active(x>0 and x<=30):	send_command dvTP[x],"'^TXT-',itoa(MIX_NAME),',0,',MXR_VOL[x].name"
				active(x>30 and x<=60):	send_command dvTP[x-30],"'^TXT-',itoa(MIX_NAME+1),',0,',MXR_VOL[x].name"
				active(x>60 and x<=90):	send_command dvTP[x-60],"'^TXT-',itoa(MIX_NAME+2),',0,',MXR_VOL[x].name"
			}
		}
		else 
		{
//			select
//			{
//				active(x>0 and x<=30):	send_command dvTP[x],"'^TXT-',itoa(MIX_NAME),',0,Level ',itoa(x)"
//				active(x>30 and x<=60):	send_command dvTP[x-30],"'^TXT-',itoa(MIX_NAME+1),',0,Level ',itoa(x)"
//				active(x>60 and x<=90):	send_command dvTP[x-60],"'^TXT-',itoa(MIX_NAME+2),',0,Level ',itoa(x)"
//			}
		}
	}
}
