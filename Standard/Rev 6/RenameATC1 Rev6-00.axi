		...
		switch(channel.channel)
		{
			...
			case RNM_KEYBRD_RCVD:
			{
				if(nSetSpeedDial1Name)
				{
					...
					off[nSetSpeedDial1Name]
					send_command dvTP[get_last(vdvRenaming)],"'@AKB-',sdATC1[nSetSpeedDial1Number].number,';Input Speed Dial Number'" //Pop up the keypad so the user can input a speed dial number				
				}
				else if (nSetSpeedDial1Number)
				{
					remove_string(cTPResponse,'KEYB-',1)	//Remove the Prefix
...
button_event[dvTP_ATC[1],ATC_SPEEDDIAL]
{
	...
	hold[15]:
	{
		...
		send_command button.input.device,"'@AKB-',sdATC1[nSetSpeedDial1Name].name,';Input Speed Dial Name'" //Pop up the keypad so the user can input a speed dial number
	}
	release:
	{
