MODULE_NAME='Biamp Speed Dial Rev5-00'(DEV vdvTP, DEV vdvATC, DEV dvATC, CHAR cAddr, INTEGER nInstID)
(***********************************************************)
...
define_module 'Biamp Speed Dial Rev5-00' atc1(dvTP_DEV[1],vdvDEV1,dvBiamp,cAddr,nInstID) 
...
data_event[dvTP]
{
	string:
	{
		send_string vdvDEV[1],"data.text"
	}
}
...
non_volatile	long		lSpeedDialTime[]={100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100}
...
DEFINE_FUNCTION Parse(CHAR cCompStr[100])
{
	...
	stack_var char cText[30]
	//STACK_VAR INTEGER nHook
	if(find_string(cCompStr,"'TISDLABEL ',itoa(nInstID),' '",1))
	{
		remove_string(cCompStr,"'TISDLABEL ',itoa(nInstID),' '",1)
	...
	{
		remove_string(cCompStr,"'TISDENTRY ',itoa(nInstID),' '",1)
		nPos=atoi(left_string(cCompStr,find_string(cCompStr,' ',1)-1))
		remove_string(cCompStr,' ',1)
		cText=left_string(cCompStr,find_string(cCompStr,"$0D",1)-1)
	...
	string:
	{
		...
		else if (left_string(cTPResponse,5)='KEYB-')
		{
			if(nSetSpeedDialNumber)
			{
				...
				if (length_string(cTPResponse)>15)	//Max length on the number is 10 characters
				{
					set_length_string(cTPResponse,15)
				}
				spdMain[nSetSpeedDialNumber].number=cTPResponse
				send_string dvATC,"'SETD ',cAddr,' TISDENTRY ',itoa(nInstID),' ',itoa(nSetSpeedDialNumber),' ',spdMain[nSetSpeedDialNumber].number,$0A"
			...
			else if (nSetSpeedDialName)
			{
				...
				send_string dvATC,"'SETD ',cAddr,' TISDLABEL ',itoa(nInstID),' ',itoa(nSetSpeedDialName),' ',spdMain[nSetSpeedDialName].name,$0A"
...
button_event [vdvTP,ATC_SPEEDDIAL]
{
	...
	release:
	{
		if(!nSetSpeedDialNumber)
		{
			send_string dvATC,"'DIAL ',cAddr,' TISPEEDDIAL ',itoa(nInstID),' ',itoa(get_last(ATC_SPEEDDIAL)),$0A"
		}
	}
}
...
{
	send_string dvATC,"'GETD ',cAddr,' TISDLABEL ',itoa(nInstID),' ',itoa(timeline.sequence),$0A"
	send_string dvATC,"'GETD ',cAddr,' TISDENTRY ',itoa(nInstID),' ',itoa(timeline.sequence),$0A"
}
(***********************************************************)
