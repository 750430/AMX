MODULE_NAME='Tandberg C Series Rev5-04'(DEV vdvTP, DEV vdvVTC, DEV dvVTC)
(***********************************************************)
...
(***********************************************************)
(* REV HISTORY:                
Rev 3 Adds Key Preview to the panel
                            *)
(***********************************************************)
#INCLUDE 'HoppSNAPI Rev5-08.axi'

//define_module 'Tandberg C Series Rev5-04' vtc1(vdvTP_VTC1,vdvVTC1,dvVTC)
...
define_function parse(cMsg[100])
{
	select
	{
		active(find_string(cMsg,"'Audio Microphones Mute:'",1)): 
		{
			...
			if(find_string(cMsg,"'On'",1))  on[nPriv]
			if(find_string(cMsg,"'Off'",1))  off[nPriv]
		}
...
define_function key(char cKey[])
{
	switch(cKey)
	{
		case '#': SEND_STRING dvVTC,"'xCommand Key Click Key:Square Duration:1',$0D,$0A"
		case '*': SEND_STRING dvVTC,"'xCommand Key Click Key:Star Duration:1',$0D,$0A"
		default: SEND_STRING dvVTC,"'xCommand Key Click Key:',cKey,' Duration:1',$0D,$0A"
	}
	send_string 0,"'key ',cKey,$0D,$0A"
	cKeyPreview="cKeyPreview,cKey"
	send_string 0,"'keypreview=',cKeyPreview"
	send_command vdvTP,"'^TXT-1,0,',cKeyPreview"
	cancel_wait 'New Key'
	wait 20 'New Key' 
	{
		cKeyPreview=''
...
{
	send_string dvVTC, "'xCommand Key Click Key:C Duration:1',$0D,$0A"
...
BUTTON_EVENT [vdvTP, btn_VTC]
{
	...
	HOLD[2,repeat]:
	{
		...
		if(button.holdtime>3000 and !nSetPreset)
		{
			IF ((nBtn = VTC_CAM_PRESET1 || nBtn = VTC_CAM_PRESET2 || nBtn = VTC_CAM_PRESET3 || 
					nBtn = VTC_CAM_PRESET4 || nBtn = VTC_CAM_PRESET5 || nBtn = VTC_CAM_PRESET6))
			{
...
CHANNEL_EVENT [vdvVTC, 0]
{
	ON:
	{
		...
		SWITCH (nChnl)
		{
			...
			case VTC_GRAPHICS: 
			{
				[vdvTP,VTC_GRAPHICS]=![vdvTP,VTC_GRAPHICS]
			...
			CASE VTC_PRES_1: send_string dvVTC,"'xConfiguration Video DefaultPresentationSource: 1',$0D,$0A"
			CASE VTC_PRES_2: send_string dvVTC,"'xConfiguration Video DefaultPresentationSource: 2',$0D,$0A"
			CASE VTC_PRES_3: send_string dvVTC,"'xConfiguration Video DefaultPresentationSource: 3',$0D,$0A"
			CASE VTC_PRES_4: send_string dvVTC,"'xConfiguration Video DefaultPresentationSource: 4',$0D,$0A"
			CASE VTC_PRES_5: send_string dvVTC,"'xConfiguration Video DefaultPresentationSource: 5',$0D,$0A"
