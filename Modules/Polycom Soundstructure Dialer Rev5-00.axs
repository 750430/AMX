MODULE_NAME='Polycom Soundstructure Dialer Rev5-00'(DEV vdvTP, DEV vdvATC, DEV dvATC)
(***********************************************************)
...
DEFINE_FUNCTION Parse(CHAR cCompStr[100])
{
	...
	IF(FIND_STRING(cCompStr,'val phone_connect "Phone Out"',1)) 
	{
		REMOVE_STRING(cCompStr,'val phone_connect "Phone Out" ',1)
...
DEFINE_FUNCTION Key(CHAR nVal[1])
{
	...
	ELSE 
	{
		...
		SEND_STRING dvATC,"'set phone_dial "Phone Out" "',nVal,'"',$0D"
	}
}
DEFINE_FUNCTION OnPush(INTEGER nCmd)
{
	SWITCH(nCmd)
	{
		...
		CASE ATC_ANSWER: 		SEND_STRING dvATC,"'set phone_connect "Phone Out" 1',$0D"
		CASE ATC_HANGUP: 		
		{
			cPhoneNum=''
			SEND_STRING dvATC,"'set phone_connect "Phone Out" 0',$0D"	
			SEND_COMMAND vdvTP,"'!T',1,cPhoneNum"	
		}
		CASE ATC_QUERY: 		SEND_STRING dvATC,"'get phone_connect "Phone Out"',$0D"
		CASE ATC_FLASH:			SEND_STRING dvATC,"'set phone_flash "Phone Out"',$0D"
		CASE ATC_DIAL: 
		{
			SEND_STRING dvATC,"'set phone_connect "Phone Out" 1',$0D"
			IF(LENGTH_STRING(cPhoneNum))
			{	
				SEND_STRING dvATC,"'set phone_dial "Phone Out" "',cPhoneNum,'"',$0D"
			}
