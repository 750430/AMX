module_name='Polycom SoundStructure Vol Control Rev6-00'(dev dvTP[], dev vdvMixer[], dev vdvMixer_FB[], dev dvMixer)
(***********************************************************)
...
Baud rate is selectable within Soundstructure

For IP Connections, use port 52774
	
	define_module 'Polycom SoundStructure Vol Control Rev6-00' mxr1(vdvTP_VOL,vdvMixer,vdvMixer_FB,dvSoundStructure) 
...
volatile 		char 		cMteStr[2][NumVolBars][50]
...
define_variable //Debugging Variables

integer tlPollMixQueryActive
integer tlMixQueryActive
...
define_function query_mixer()
{
	...
	for(x=1;x<=max_length_array(mxr_vol);x++)
	{
		if(length_string(mxr_vol[x].name)>0) nMaxVolBar=x
	}
...
define_function ramp(integer nDir, integer nI)
{
	...
	send_string dvMixer,"cLvlStr[nDir][ nI]"
}
...
channel_event[vdvMixer,0]
{
	on:	  
	{
		...
		if(length_string(mxr_vol[nActiveVolBar].name)>0)
		{
			switch(nActiveVolButton)
			{
				...
				case MIX_MUTE_TOG: 
				{
					switch(mxr_vol[nActiveVolBar].mte)
					{
						case 1: send_string dvMixer,"cMteStr[muteOff][nActiveVolBar]"
						case 0: send_string dvMixer,"cMteStr[muteOn][nActiveVolBar]"
					}
				}				
				case MIX_MUTE_OFF: 	send_string dvMixer,"cMteStr[muteOff][nActiveVolBar]"
				case MIX_MUTE_ON:   send_string dvMixer,"cMteStr[muteOn][nActiveVolBar]"	
				case MIX_QUERY: 
				{
					send_string dvMixer,"cQueryStr[nActiveVolBar]"
					send_string dvMixer,"cQueryMuteStr[nActiveVolBar]"
					send_string dvMixer,"'get fader max "',MXR_VOL[nActiveVolBar].name,'"',$0D"
					send_string dvMixer,"'get fader min "',MXR_VOL[nActiveVolBar].name,'"',$0D"
				}
				case MIX_UPDATE_ALL:
				{
					query_mixer()
...
{
	if(length_string(mxr_vol[timeline.sequence].name)>0) pulse[vdvMixer[timeline.sequence],MIX_QUERY]
}
...
tlPollMixQueryActive=timeline_active(tlPollMixQuery)
tlMixQueryActive=timeline_active(tlMixQuery)
