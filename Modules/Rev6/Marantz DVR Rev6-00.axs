MODULE_NAME='Marantz DVR Rev6-00'(DEV dvTP[], DEV vdvRec, dev vdvRec_FB, DEV dvRec)
(***********************************************************)
...
define_module 'Marantz DVR Rev6-00' DVR1(dvTP_DEV[1],vdvDEV1,vdvDEV1_FB,dvMarantz)
...
DEFINE_FUNCTION Parse(CHAR cCompStr[100])
{
	...
	SELECT
	{
		ACTIVE(FIND_STRING(cCompStr,"'@0TR'",1)):
		{
			...
			SEND_COMMAND dvTP,"'^TXT-',ITOA(DVR_TRACK_TXT),',1&2,',cTemp"
		}	
		ACTIVE(FIND_STRING(cCompStr,'@0TI',1)):
		{
			...
			SEND_COMMAND dvTP,"'^TXT-',ITOA(DVR_COUNTER_TXT),',1&2,',cHour,':',cMins,':',cSecs"
		}
	...
	switch(nDeckStatus)
	{
		case stRecord: send_command dvTP,"'^TXT-',itoa(DVR_STATUS_TXT),',0,Recording in Progress'"
		case stRecordPause: send_command dvTP,"'^TXT-',itoa(DVR_STATUS_TXT),',0,Recording Paused - Press Record to Resume'"
		case stStop: send_command dvTP,"'^TXT-',itoa(DVR_STATUS_TXT),',0,Recorder Idle'"
		case stPlay: send_command dvTP,"'^TXT-',itoa(DVR_STATUS_TXT),',0,Playing . . .'"
		case stPause: send_command dvTP,"'^TXT-',itoa(DVR_STATUS_TXT),',0,Recorder Paused'"
		case stOther: send_command dvTP,"'^TXT-',itoa(DVR_STATUS_TXT),',0,'"
	}
}

DEFINE_FUNCTION OnPush(INTEGER nIndex)
{
	SEND_STRING dvRec,"cCmdStr[nIndex]"
	SEND_STRING dvRec,"cPollStr[PollStatus]"
	send_string dvRec,"cPollStr[PollTrack]"
}
...
TIMELINE_EVENT[lTLPoll]
{
	select
	{
		...
		active(1):
		{
			SEND_STRING dvRec,"cPollStr[PollCounter]"
		}
	}
}
CHANNEL_EVENT[vdvRec,0]
{
	ON:	IF(channel.channel<200) OnPush(channel.channel)
}
BUTTON_EVENT [dvTP,nPanelBtn]
{
	PUSH:	
	{
		TO[button.input.device,button.input.channel]
		ON[vdvRec,GET_LAST(nPanelBtn)]	
	}
	RELEASE: OFF[vdvRec,GET_LAST(nPanelBtn)]
}
...
[dvTP,DVR_REC_ON]=nDeckStatus=stRecord
[dvTP,DVR_REC]=(nDeckStatus=stRecord or nDeckStatus=stRecordPause)
[dvTP,DVR_PLAY]=nDeckStatus=stPlay
[dvTP,DVR_PAUSE]=(nDeckStatus=stPause or nDeckStatus=stRecordPause)
[dvTP,DVR_STOP]=nDeckStatus=stStop
(***********************************************************)
(*                     END OF PROGRAM                      *)
