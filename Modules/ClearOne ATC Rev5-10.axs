module_name='ClearOne ATC Rev5-10'(dev dvTP, dev vdvATC, dev dvXAP, char cAddr[])
(***********************************************************)
...
define_function parse(char cCompStr[100])
{
	select
	{
		active(find_string(cCompStr,"'#',cAddr,' XTE 1 Z 1'",1)):
		{
			nHook = 1
		}
		active(find_string(cCompStr,"'#',cAddr,' XTE 1 Z 0'",1)):
		{
			nHook = 0
		}
	}
...
define_function key(char c[])
{
	...
	else send_string dvXAP,"'#',cAddr,' XDIAL 1 Z ',c,$0D"
}
...
channel_event[vdvATC,0]
{
	on:
	{
		switch(channel.channel)
		{
			...
			{
				cPhoneNum=''
				send_string dvXAP,"'#',cAddr,' XTE 1 Z 1',$0D"
			}
			case ATC_HANGUP: 
			{
				send_string dvXAP,"'#',cAddr,' XTE 1 Z 0',$0D"
				cPhoneNum=''
				send_command dvTP,"'^TXT-1,',cPhoneNum"
			}
			case ATC_FLASH: send_string dvXAP,"'#',cAddr,' XHOOK Z 1',$0D"
			case ATC_QUERY: send_string dvXAP,"'#',cAddr,' XTE Z 1',$0D"
			case ATC_DIAL:
			{
				switch([vdvATC,ATC_OFF_HOOK])
				{
					case 0:
					{
						...
						{
							send_string dvXAP,"'#',cAddr,' XDIAL 1 Z ',cPhoneNum,$0D"
						}
						else
						{
							send_string dvXAP,"'#',cAddr,' XTE 1 Z 1',$0D"
