MODULE_NAME='Digital Projection Titan 660 2D Rev5-01'(DEV vdvTP, DEV vdvProj, DEV dvProj)
(***********************************************************)
...
(*   
	...
	define_module 'Digital Projection Titan 660 2D Rev5-01' proj1(vdvTP_DISP1,vdvDISP1,dvProj)
*)
...
volatile		integer		nInput[]={VD_SRC_VGA1,VD_SRC_DVI1,VD_SRC_HDMI1,VD_SRC_AUX1}
	...
	SELECT
	{
		ACTIVE(FIND_STRING(cCompStr,cAckStr[AckPower],1)):
		{
			...
			switch(nCmd)
			{
				...
				case VD_SRC_AUX1:
				{
					if(nActivePower<>VD_WARMING) nWarming=0
					nActivePower=VD_WARMING
				}
				case VD_PWR_OFF:
				{
					if(nActivePower<>VD_COOLING) nCooling=0
		...
		{			
			log("'Proj Rx: Power On'")
			switch(nActivePower)
			{ 
				case VD_COOLING:
				{
					//Do Nothing.  If this happens it's because we've tried to shut it down while it's still booting.
					//It'll stay on cooling for a long time then shut down eventually.
				}
				case VD_WARMING:
				{
					nWarming++
					log("'Warming ',itoa(nWarming)")
					if(nWarming>15)
					{
						nWarming=0
				...
				case VD_PWR_ON:
				case VD_PWR_OFF:
				default: nActivePower=VD_PWR_ON
			}
		}
		ACTIVE(FIND_STRING(cCompStr,cRespStr[VD_PWR_OFF],1)):
		{	
			log('Proj Rx: Power Off')
			switch(nActivePower)
			{ 
				case VD_WARMING:
				{
					//Do Nothing.  If this happens it's because we've tried to start it up while it's still shutting down
					//It'll stay on warming for a long time then start up eventually.
				}
				case VD_COOLING:
				{
					nCooling++
					log("'Cooling ',itoa(nCooling)")
					if(nCooling>15)
					{
						nCooling=0
				...
				case VD_PWR_ON:
				case VD_PWR_OFF:
				default: nActivePower=VD_PWR_OFF
			}
		}
		ACTIVE(FIND_STRING(cCompStr,"$74,$50,$01,$00,$00,$00,$00,$00,$00,$00,$00,$11,$00,$00,$00,$00,$70,$46,$01,$40,$00,$00,$00,$00,$00,$00,$00,$05,$00,$00,$00,$00",1)):
		{
			...
			log("'Proj Rx: Input ',itoa(hextoi(cInput))")
			IF(cInput=$04)
			{
...
define_function log (char cMsg[100])
{
	send_string 0,"date,' ',time,' ',cMsg"
}
...
TIMELINE_EVENT[lTLPoll]				//Projector Polling
{
	...
	switch(nPollType)
	{
		case PollInput: log('Proj Tx: Poll Input')
		case PollPower: log('Proj Tx: Poll Power')
	}
}
	...
	SWITCH(TIMELINE.SEQUENCE)
	{
		CASE 1:	//first time
		{
			SWITCH(nCmd)
			{
				CASE VD_PWR_ON:
				{
					log('Proj Tx: Power On')
					SEND_STRING dvProj,cCmdStr[nCmd]
					nPollType = PollPower
				}
				CASE VD_PWR_OFF: 
				{
					log('Proj Tx: Power Off')
					SEND_STRING dvProj,cCmdStr[nCmd]
					nPollType = PollPower
				}
				CASE VD_SRC_VGA1:
				...
				CASE VD_SRC_AUX1:
				{
					...
					else IF(nActivePower=VD_PWR_OFF|| nActivePower=VD_COOLING)
					{
						log('Proj Tx: Power On')
				...
				CASE VD_MUTE_TOG:
				{
					IF(nActivePower=VD_PWR_ON)
					{
						IF(nActiveMute=VD_MUTE_ON) nCmd=VD_MUTE_OFF
...
{
	[dvProj,nPower[x]]=nActivePower=nPower[x]
	[vdvTP,nPower[x]]=nActivePower=nPower[x]
}

for(x=1;x<=length_array(nInput);x++)
{
	[dvProj,nInput[x]]=nActiveInput=nInput[x]
	[vdvTP,nInput[x]]=nActiveInput=nInput[x]
}

for(x=1;x<=length_array(nMute);x++)
{
	[dvProj,nMute[x]]=nActiveMute=nMute[x]
	[vdvTP,nMute[x]]=nActiveMute=nMute[x]
}
