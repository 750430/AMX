MODULE_NAME='Sharp LCD LC-50LE650U Rev5-02'(dev dvTP, dev vdvLCD, dev dvLCD)
(***********************************************************)
...
(*  
	...
	define_module 'Sharp LCD LC-50LE650U Rev5-02' LCD1(vdvTP_DISP1,vdvDISP1,dvLCD1)
...
long 		lCmdArray[]				= {2000,2000}
long		lErrorArray[]			= {10000}	
...
DEFINE_FUNCTION Parse(CHAR cCompStr[100])
{
	...
	if(find_string(cCompStr,"'ERR'",1)) 
	{
		if(timeline_active(tlError)) timeline_kill(tlError)
		timeline_create(tlError,lErrorArray,length_array(lErrorArray),timeline_relative,timeline_repeat)
	}
...
TIMELINE_EVENT[tlCmd]
{
	if(!timeline_active(tlError))
	{
		SWITCH(TIMELINE.SEQUENCE)
		{
			...
			CASE 2:	//2nd time
			{
				if(timeline.repetition>2) command_to_display()
			}
		}
	}
}
...
timeline_event[tlError]
{
	send_string dvLCD,cPollStr[PollPwr]
}
