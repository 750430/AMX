MODULE_NAME='ClearOne Volume Rev5-02'(DEV vdvTP[], DEV vdvXAP[], DEV dvXAP)
(***********************************************************)
(*  FILE_LAST_MODIFIED_ON: 04/27/2012  AT: 12:59:40        *)
(***********************************************************)
(***********************************************************)
(* System Type : NetLinx                                   *)
(***********************************************************)
...
VOL[1].addr = '70' 			//Address of XAP unit
...
define_module 'ClearOne Volume Rev5-02' mxr1(vdvTP_VOL,vdvMXR,dvClearOne)
*)
...
CHAR cMteStr[40][100]
CHAR cLvlStr[40][100]
CHAR cGainStr[40][100]

char cJeffString[40][10]
...
DEFINE_FUNCTION Parse(CHAR cCompStr[100])
{
	...
	FOR (nLvl=1; nLvl<max_length_array(mxr_vol); nLvl++)
	{
		SELECT
		{
			ACTIVE(FIND_STRING(cCompStr,"'user:'",1)): send_string dvXAP,"'clearone',$0D,$0A"
			ACTIVE(FIND_STRING(cCompStr,"'pass:'",1)): send_string dvXAP,"'converge',$0D,$0A"
...
DEFINE_CALL 'INIT_STRINGS'
{
	...
	FOR (X=1; X<=max_length_array(MXR_VOL); X++)
	{
		...
		if (mxr_vol[x].type='I' and atoi(mxr_vol[x].chan)<9) cGainStr[X]="'#',MXR_VOL[X].addr,$20,'GAIN',$20,MXR_VOL[X].chan,$20,'M',$20"
...
TIMELINE_EVENT[lFB]
{
	...
	FOR(X=1; X<max_length_array(mxr_vol); X++)
	{
		...
		[vdvTP[X], MIX_MUTE_OFF_FB] = !(nMteVal[X])
		[vdvTP[X], MIX_MUTE_ON_FB] = nMteVal[X]
		[vdvXAP[X], MIX_MUTE_OFF_FB] = !(nMteVal[X])
		[vdvXAP[X], MIX_MUTE_ON_FB] = nMteVal[X]
	}
}
