module_name='Polycom HDX Series Rev5-01'(dev dvTP,dev vdvVTC,dev dvVTC)
(***********************************************************)
...
//SET BAUD 9600,N,8,1
//define_module 'Polycom HDX Series Rev5-01' vtc1(vdvTP_VTC1,vdvVTC1,dvVTC)
...
{
	send_string dvVTC, "'button left',$0D"
...
define_function OnPush (integer nIndex)
{
	switch(nIndex) 
	{
		...
		case VTC_SEND_PC: 			send_string dvVTC,"'vcbutton play 3',$0D"
		...
		case VTC_CONTENT_ON:		send_string dvVTC,"'vcbutton play 3',$0D"
...
button_event[dvTP,VTC_DELETE]
{
	...
	hold[3,repeat]:
	{
		...
		{
			send_string dvVTC, "'button back',$0D"
			if(length_string(cKeyPreview)>0)
			{
				cancel_wait 'New Key'
				cKeyPreview=''
