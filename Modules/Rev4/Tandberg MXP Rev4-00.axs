MODULE_NAME='Tandberg MXP Rev4z-00'(DEV vdvTP, DEV vdvVTC, DEV dvVTC)
(***********************************************************)
...
//define_module 'Tandberg MXP Rev4-00' vtc1(vdvTP_VTC1,vdvVTC1,dvVTC)
//Set Baud 9600,N,8,1
(***********************************************************)
	...
	STRING:
	{
		...
		SELECT
		{
			ACTIVE(FIND_STRING(cVTC_Buff,"'xconfiguration audio microphones mode:'",1)): 
			{
				...
				IF(FIND_STRING(cVTC_Buff,"'on'",1))  ON[vdvVTC, VTC_PRIVACY_OFF_FB]
				IF(FIND_STRING(cVTC_Buff,"'off'",1))  ON[vdvVTC, VTC_PRIVACY_ON_FB]
			}
			ACTIVE(1):	cVTC_Buff=''
		}		
	}
