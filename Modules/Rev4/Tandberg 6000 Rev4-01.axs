MODULE_NAME='Tandberg 6000 Rev4-01'(DEV vdvTP, DEV vdvVTC, DEV dvVTC)
(***********************************************************)
...
DEFINE_FUNCTION Parse(CHAR cCompStr[100])
{
	select
	{
		...
		active(find_string(cCompStr,"'vidin'",1)):
		{
			remove_string(cCompStr,"'vidin '",1)
			nSrc=atoi(left_string(cCompStr,1))
			send_string 0,"'nSrc=',itoa(nSrc)"
		}
	}
}
...
BUTTON_EVENT [vdvTP,0]
{
	PUSH:		
	{
		...
		switch(nBtn)
		{
			...
			default:				to[vdvVTC,nBtn]
		}
		send_string 0,"'Pressed ',itoa(nBtn)"
	}
	hold[5,repeat]:
	{
		send_string 0,"'Holding ',itoa(nBtn)"
		switch(nBtn)
		{
			case VTC_ZOOM_IN:		send_string dvVTC,"'key z+',$0D,$0A"
			case VTC_ZOOM_OUT:		send_string dvVTC,"'key z-',$0D,$0A"
...
CHANNEL_EVENT [vdvVTC,0]
{
	ON:
	{
		...
		SWITCH (nChnl)
		{
			...
			CASE VTC_NR_VID1:		send_string dvVTC,"'vidin 1',$0D,$0A"
			CASE VTC_NR_VID2:		send_string dvVTC,"'vidin 2',$0D,$0A"
			CASE VTC_NR_VID3:		send_string dvVTC,"'vidin 3',$0D,$0A"
			CASE VTC_NR_VID4:		send_string dvVTC,"'vidin 4',$0D,$0A"
			CASE VTC_NR_VID5:		send_string dvVTC,"'vidin 5',$0D,$0A"
...
timeline_event[lPoll]
{
	...
	send_string dvVTC,"'screensaver off',$0D,$0A"
}
